...
(function checkLocation() {
  const granted = localStorage.getItem('bs-location-granted');
  if (granted === 'true') return;

  const banner = document.createElement('div');
  banner.id = 'location-banner';
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999;background:#C96A00;color:#fff;padding:14px 20px;text-align:center;font-size:14px;font-weight:500;display:flex;align-items:center;justify-content:center;gap:12px';
  banner.innerHTML = `
    <span>📍 BreatheSafe needs your location to show live AQI data.</span>
    <button id="enableLocBtn" style="background:#fff;color:#C96A00;border:none;border-radius:6px;padding:6px 14px;font-weight:700;cursor:pointer;font-size:13px;">Enable Location</button>
  `;
  document.body.prepend(banner);

  document.addEventListener('click', function handler(e) {
    if (e.target.id !== 'enableLocBtn') return;
