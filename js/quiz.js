...
/* ── TRANSITION HELPER ── */
function transitionCard(newCard) {
...
async function saveAndFinish() {
  ...
  showLocationCard(name);
}

/* ── LOCATION CARD ── */
function showLocationCard(name) {
  const card = document.createElement('div');
  card.className = 'quiz-card quiz-permission';
  card.innerHTML = `
    <span class="done-icon">📍</span>
    <p class="quiz-eyebrow">Required</p>
    <h2 class="quiz-question">BreatheSafe needs your location</h2>
    <p class="done-sub">Without your location, live AQI data,<br>air quality scores, and AI insights won't work.</p>
    <div class="permission-btns">
      <button class="quiz-next active" id="allowLocation">Allow Location</button>
      <button class="quiz-skip" id="skipLocation">Skip anyway</button>
    </div>
    <p class="permission-note">⚠️ Skipping will limit most features. You can enable it later in browser settings.</p>
  `;
  transitionCard(card).then(() => {
    document.getElementById('allowLocation').addEventListener('click', () => {
      navigator.geolocation.getCurrentPosition(
        pos => {
          localStorage.setItem('bs-lat', pos.coords.latitude);
          localStorage.setItem('bs-lng', pos.coords.longitude);
          localStorage.setItem('bs-location-granted', 'true');
          showNotifCard(name);
        },
        () => {
          localStorage.setItem('bs-location-granted', 'false');
          showNotifCard(name);
        }
      );
    }, { once: true });
    ...
    document.getElementById('skipLocation').addEventListener('click', () => {
      localStorage.setItem('bs-location-granted', 'skipped');
      showNotifCard(name);
    }, { once: true });
  });
}
...
/* ── NOTIFICATION CARD ── */
function showNotifCard(name) {
  const card = document.createElement('div');
  card.className = 'quiz-card quiz-permission';
  card.innerHTML = `
    <span class="done-icon">🔔</span>
    <p class="quiz-eyebrow">Stay on track</p>
    <h2 class="quiz-question">Enable inhaler reminders?</h2>
    <p class="done-sub">We'll remind you to take your inhaler<br>so you never miss a dose.</p>
    <div class="permission-btns">
      <button class="quiz-next active" id="allowNotif">Enable Reminders</button>
      <button class="quiz-skip" id="skipNotif">Skip for now</button>
    </div>
    <p class="permission-note">You can set up your reminder schedule on the dashboard.</p>
  `;
  transitionCard(card).then(() => {
    document.getElementById('allowNotif').addEventListener('click', () => {
      if ('Notification' in window) {
        Notification.requestPermission().then(permission => {
          localStorage.setItem('bs-notif-granted', permission);
          showDoneCard(name);
        });
      } else {
        localStorage.setItem('bs-notif-granted', 'unsupported');
        showDoneCard(name);
      }
    }, { once: true });

    document.getElementById('skipNotif').addEventListener('click', () => {
      localStorage.setItem('bs-notif-granted', 'skipped');
      showDoneCard(name);
    }, { once: true });
  });
}
